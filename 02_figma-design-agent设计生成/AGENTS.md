...
生成方案图时必须使用 image gen 指令直接生成 PNG、JPEG 或 WebP 等位图。禁止使用 SVG、HTML/CSS、Canvas、Figma shape 脚本或任何矢量/代码绘图方式生成方案图；也禁止先画 SVG 再转成 PNG 冒充 image gen 方案图。运行记录必须写明方案图 `outputType`，非位图或来源不明时不得进入用户确认。
...
已有页面新增模块和局部布局匹配，最后必须执行 `CHECKLISTS.md` 的“页面级规范硬性机械校验”。该校验按 [figma design agent 页面级设计规范.md](figma%20design%20agent%20%E9%A1%B5%E9%9D%A2%E7%BA%A7%E8%AE%BE%E8%AE%A1%E8%A7%84%E8%8C%83.md) 的目标页面章节逐项验证，且硬性规则优先、机械验证优先：字号、颜色、间距、内外边距、圆角、描边、阴影、模块尺寸和页面固定布局必须用 Figma 节点属性、变量绑定、脚本量测或等价可复核证据证明；无法量测或未记录实际值时，不得交付。

最低通过条件：

- 所有必需内容已出现。
- 没有占位文本和需求说明。
- 颜色、字号、字重、间距、内外边距、圆角、描边、阴影符合产品级和页面级规范，并在运行记录中有逐项机械校验证据。
- 文本没有重叠、裁切或溢出。
- frame 没有裁切内容。
- 新增模块 / 页面与已确认方案图方向一致。
